...
uint256 hashGenesisBlock("0x1beebea34a3b5131b068eb32e28e3384f83e6e3c812bf0441a7b6907a254edcd");
...
const string strMessageMagic = "Debitcoin Signed Message:\n";
...
{
    // Debitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Debitcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Debitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x3bdfcb7afbf2e792f27d2c91c6564eea90b506611f0ca679eebc3201b1dedf19"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd7, 0xc9, 0xad, 0xab };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DebitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DebitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DebitcoinMiner(CWallet *pwallet)
{
    printf("DebitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Debitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DebitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DebitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DebitcoinMiner, pwallet));
}
