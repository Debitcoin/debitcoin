...
// Copyright (c) 2011-2012 Debitcoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("Debitcoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DebitcoinTEST0\r");
            Send(hSocket, "WHO #DebitcoinTEST0\r");
        } else {
            // randomly join #Debitcoin00-#Debitcoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Debitcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Debitcoin%02d\r", channel_number).c_str());
        }
